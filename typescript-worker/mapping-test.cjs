...
(async () => {
  try {
    ...
    const invalidReturnPlan=await request('mappingGeneration');for(const file of invalidReturnPlan.files){fs.writeFileSync(path.join(root,file.path),file.content);}
