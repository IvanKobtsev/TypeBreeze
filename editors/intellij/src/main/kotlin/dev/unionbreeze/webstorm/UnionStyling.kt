...
class UnionBreezeColorSettingsPage:ColorSettingsPage {
    ...
    override fun getDemoText()="""type Status = <unionDeclaration>'draft'</unionDeclaration> | 'published';
const status: Status = <unionUsage>'draft'</unionUsage>;
const ordinary = 'draft';"""
    override fun getAdditionalHighlightingTagToDescriptorMap()=mapOf("unionDeclaration" to UnionBreezeColors.DECLARATION,"unionUsage" to UnionBreezeColors.USAGE)
    override fun getAttributeDescriptors()=arrayOf(AttributesDescriptor("Union member declaration",UnionBreezeColors.DECLARATION),AttributesDescriptor("Union member usage",UnionBreezeColors.USAGE))
...
class UnionBreezeAnnotator:Annotator {
    override fun annotate(element:PsiElement,holder:AnnotationHolder){
        ...
        val settings=UnionBreezeSettings.instance.state;val key=when(resolved.kind){"declaration"->if(settings.styleDeclarations)UnionBreezeColors.DECLARATION else return;"usage"->if(settings.styleUsages)UnionBreezeColors.USAGE else return;else->return}
...
class UnionBreezeSettings:PersistentStateComponent<UnionBreezeSettings.Options> {
    data class Options(var styleDeclarations:Boolean=true,var styleUsages:Boolean=true)
...
class UnionBreezeConfigurable:BoundConfigurable("UnionBreeze") {
    override fun createPanel()=panel { val settings=UnionBreezeSettings.instance.state;row{checkBox("Style union member declarations").bindSelected(settings::styleDeclarations)};row{checkBox("Style contextual union member usages").bindSelected(settings::styleUsages)} }
